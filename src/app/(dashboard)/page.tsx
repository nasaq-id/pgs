        ...
        <div className="space-y-8">
          ...
          <div className="bg-white rounded-[2rem] border border-slate-100 p-6 shadow-sm space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] font-black px-2 py-0.5 bg-teal-50 text-teal-600 rounded">Bulan Ini</span>
            </div>
            ...
            <div className="grid grid-cols-7 gap-1 text-center">
              ...
              {Array.from({ length: new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0).getDate() }).map((_, idx) => {
                const today = idx + 1 === new Date().getDate()
                return (
                  <div key={idx} className={`aspect-square flex items-center justify-center text-xs font-semibold rounded-full transition-all ${
                    today ? "bg-slate-950 text-white font-extrabold shadow-sm" : "text-slate-700 hover:bg-slate-50 cursor-pointer"
                  }`}>{idx + 1}</div>
                )
              })}
            </div>
