...
function Input({ className, type, lang, placeholder, ...props }: React.ComponentProps<"input">) {
  const isDate = type === "date"
  return (
    <InputPrimitive
      type={type}
      data-slot="input"
      lang={isDate ? (lang || "id-ID") : lang}
      ...
      className={cn(
        ...
        isDate && "[&::-webkit-calendar-picker-indicator]:cursor-pointer [&::-webkit-calendar-picker-indicator]:opacity-70 hover:[&::-webkit-calendar-picker-indicator]:opacity-100 dark:[&::-webkit-calendar-picker-indicator]:invert",
        className
      )}
      {...props}
    />
