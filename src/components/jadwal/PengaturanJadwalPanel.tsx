...
import { Loader2, Plus, Trash2, Copy, Clock, BookOpen, Flag, Coffee, Sparkles, ChevronDown, Settings, Save, RotateCcw, AlertTriangle } from "lucide-react"
...
export default function PengaturanJadwalPanel({ onDone }: Props) {
  ...
  const clearTimelineDays = api.pengaturanJadwal.clearTimelineDays.useMutation({
    onSuccess: () => {
      utils.pengaturanJadwal.getTimeline.invalidate()
    },
  })

  const clearAllTimeline = api.pengaturanJadwal.clearAllTimeline.useMutation({
    onSuccess: () => {
      utils.pengaturanJadwal.getTimeline.invalidate()
    },
  })
  ...
  const [resetChecklist, setResetChecklist] = useState<Record<string, boolean>>(() =>
    Object.fromEntries(ALL_DAYS.map((d) => [d.value, true]))
  )
  ...
  const openResetModal = useCallback(() => {
    const isCurrentDayFilled = (itemsByDay.get(timelineHari)?.length ?? 0) > 0
    setResetChecklist(Object.fromEntries(ALL_DAYS.map((d) => [d.value, d.value === timelineHari && isCurrentDayFilled])))
    setShowResetModal(true)
  }, [itemsByDay, timelineHari])

  const handleConfirmReset = useCallback(async () => {
    const selected = ALL_DAYS.map((d) => d.value).filter((d) => resetChecklist[d])
    if (selected.length === 0) return
    setSaving(true)
    try {
      await clearTimelineDays.mutateAsync({ hari: selected as any })
      toast.success(`${selected.length} hari berhasil di-reset (hapus permanen)`)
      setShowResetModal(false)
    } catch {
      toast.error("Gagal mereset jadwal")
    } finally {
      setSaving(false)
    }
  }, [resetChecklist, clearTimelineDays])
  ...
  const recalculateAndSaveTimeline = async (
    hari: string,
    updatedItemsList: Omit<TimelineFormData, "id">[] | TimelineFormData[]
  ) => {
    // Sort items by their urutan (position)
    const sorted = [...updatedItemsList].sort((a, b) => a.urutan - b.urutan)

    let currentMinutes = timeToMinutes(jamMulai)

    for (let i = 0; i < sorted.length; i++) {
      const item = sorted[i]
      
      // Determine duration of this item
      let duration = durasiJP
      if (item.tipe !== "jp") {
        duration = timeToMinutes(item.jamSelesai) - timeToMinutes(item.jamMulai)
        if (duration <= 0) duration = 30 // fallback default
      }

      const itemStart = currentMinutes
      const itemEnd = currentMinutes + duration
      currentMinutes = itemEnd // next item starts at the end of this one

      const newJamMulai = minutesToTime(itemStart)
      const newJamSelesai = minutesToTime(itemEnd)

      // Save to database
      await upsertTimeline.mutateAsync({
        id: (item as TimelineFormData).id,
        hari: hari as any,
        tipe: item.tipe as any,
        label: item.label ?? undefined,
        jamMulai: newJamMulai,
        jamSelesai: newJamSelesai,
        urutan: i + 1, // normalize to contiguous 1-indexed urutan
        warna: (item as TimelineFormData).warna ?? undefined,
      })
    }
  }
  ...
  const handleSavePengaturan = async () => {
    ...
    try {
      ...
      for (const day of ALL_DAYS) {
        ...
        if (items.length > 0) {
          await recalculateAndSaveTimeline(day.value, items)
        }
      }

      toast.success("Pengaturan jadwal berhasil disimpan & semua hari diperbarui")
      onDone?.()
    } catch {
  ...
  const handleDeleteItem = async (id: string) => {
    setSaving(true)
    try {
      const deletedItem = timelineItems.find((t) => t.id === id)
      if (!deletedItem) return

      await deleteTimeline.mutateAsync({ id })

      const remainingItems = (itemsByDay.get(deletedItem.hari) ?? [])
        .filter((t) => t.id !== id)

      await recalculateAndSaveTimeline(deletedItem.hari, remainingItems)
  ...
  return (
    <div className="neumo-card bg-background rounded-3xl p-6 md:p-8 border-0 space-y-6 text-left animate-fade-in">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-5">
        <div>
          ...
          <p className="text-xs text-slate-400 font-semibold mt-1">
            Konfigurasi durasi jam pelajaran, jam mulai KBM, dan susunan kegiatan per hari secara dinamis.
          </p>
        </div>
        <Button
          onClick={handleSavePengaturan}
          disabled={saving || loading}
          className="flex items-center justify-center font-bold px-6 py-2.5 bg-teal-600 hover:bg-teal-700 text-white rounded-xl transition-all text-xs uppercase tracking-wider whitespace-nowrap cursor-pointer !h-10 shadow-md neumo-sm"
        >
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            <div className="bg-slate-50 dark:bg-slate-900/40 p-4 rounded-2xl border border-slate-100 dark:border-slate-800/80">
            ...
            <div className="flex items-center gap-2">
              ...
              {(itemsByDay.get(timelineHari)?.length ?? 0) > 0 && (
                <Button
                  size="sm"
                  variant="outline"
                  className="h-8 rounded-xl text-xs gap-1.5 border-rose-200 dark:border-rose-900/60 font-black uppercase tracking-wider text-rose-600 hover:text-rose-700 hover:bg-rose-50 dark:hover:bg-rose-950/20 cursor-pointer"
                  onClick={openResetModal}
                  disabled={saving || clearTimelineDay.isPending || clearTimelineDays.isPending || clearAllTimeline.isPending}
                >
      ...
      {/* Reset Modal — pilih hari mana saja yang mau direset (hard delete) */}
      {showResetModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs transition-opacity duration-300" onClick={() => !saving && setShowResetModal(false)}>
          <div className="bg-background rounded-[28px] border border-slate-100 dark:border-slate-800 shadow-2xl p-6 w-full max-w-md mx-4 animate-scale-in" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-2 mb-1">
              <div className="p-2 rounded-xl bg-rose-50 dark:bg-rose-950/30 text-rose-500 shrink-0">
                <AlertTriangle className="h-4.5 w-4.5" />
              </div>
              <h3 className="text-sm font-black text-slate-800 dark:text-white uppercase tracking-wider">
                Reset Jadwal (Hapus Permanen)
              </h3>
            </div>
            <p className="text-xs text-slate-400 font-semibold mt-1 mb-4">
              Pilih hari yang ingin dihapus. Data JP, istirahat, upacara, dan kegiatan lain akan dihapus <span className="text-rose-500 font-black">permanen</span> dan tidak bisa dikembalikan.
            </p>

            <div className="space-y-1.5 max-h-[46vh] overflow-y-auto pr-1 mb-4">
              {ALL_DAYS.map((d) => {
                const count = itemsByDay.get(d.value)?.length ?? 0
                const isActive = timelineHari === d.value
                const checked = !!resetChecklist[d.value]
                return (
                  <div
                    key={d.value}
                    onClick={() => setResetChecklist((prev) => ({ ...prev, [d.value]: !prev[d.value] }))}
                    className={`flex items-center justify-between gap-3 px-3.5 py-2.5 rounded-xl border cursor-pointer transition-all ${
                      checked
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <input
                        type="checkbox"
                        checked={checked}
                        onChange={() => setResetChecklist((prev) => ({ ...prev, [d.value]: !prev[d.value] }))}
                        className="rounded border-slate-300 text-rose-500 focus:ring-rose-500 h-4 w-4 shrink-0"
                      />
                      <span className="text-xs font-extrabold text-slate-700 dark:text-slate-300 uppercase tracking-wider">{d.label}</span>
                      {isActive && (
                        <span className="text-[9px] font-black px-1.5 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-950/50 text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">
                          Aktif
                        </span>
                      )}
                    </div>
                    <span className={`text-[9px] font-black px-2 py-0.5 rounded-full tracking-tight shrink-0 ${
                      count > 0 ? "bg-slate-200/70 dark:bg-slate-800 text-slate-600 dark:text-slate-300" : "text-slate-400 dark:text-slate-500 italic"
                    }`}>
                      {count > 0 ? `${count} Kegiatan` : "Kosong"}
                    </span>
                  </div>
                )
              })}
            ...
            <div className="flex items-center justify-between gap-2 border-t border-slate-100 dark:border-slate-800 pt-4">
              <button
                type="button"
                onClick={() => setResetChecklist(Object.fromEntries(ALL_DAYS.map((d) => [d.value, true])))}
                className="text-[10px] font-black text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 uppercase tracking-wider cursor-pointer"
              >
                Pilih Semua
              </button>
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  disabled={saving || Object.values(resetChecklist).filter(Boolean).length === 0}
                  className="bg-rose-600 hover:bg-rose-700 text-white rounded-xl h-9 px-4 text-xs font-black uppercase tracking-wider cursor-pointer"
                >
                  {saving && <Loader2 className="h-3.5 w-3.5 animate-spin mr-1.5" />}
                  Reset {Object.values(resetChecklist).filter(Boolean).length} Hari
                </Button>
              </div>
            </div>
          </div>
